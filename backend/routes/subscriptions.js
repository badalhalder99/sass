...
router.put('/update', requireTenant, async (req, res) => {
  try {
    ...
    const currentSubscription = await Subscription.findByTenantId(req.tenantId, 'mysql');
    ...
    if (!currentSubscription) {
      ...
      const result = await subscription.save('mysql');
    ...
    const subscriptionInstance = new Subscription(currentSubscription);
    ...
    await subscriptionInstance.update(updateData, 'mysql');
...
router.post('/cancel', requireTenant, async (req, res) => {
  try {
    ...
    const subscriptionInstance = new Subscription(currentSubscription);
    await subscriptionInstance.cancel('mysql');
...
router.get('/features/:feature', requireTenant, async (req, res) => {
  try {
    ...
    const subscriptionInstance = new Subscription(subscription);
